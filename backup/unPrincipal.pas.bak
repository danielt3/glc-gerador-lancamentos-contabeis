...
type
  ...
  private
    ...
    function  ExportarLancamentosExcel(Empresa, Layout: Integer; pNomeArquivo: String; pBarraProgresso: TProgressBar): Boolean;
...
function TfrmPrincipal.ExportarLancamentosExcel(Empresa, Layout: Integer; pNomeArquivo: String; pBarraProgresso: TProgressBar): Boolean;
const
...
begin
  for i := 0 to fListaCamposNome.Count - 1 do
  begin
    DataModule1.CampoLancamentoLocate(fListaCamposNome.Strings[i]);

    if (FindComponent('edtLanc_' + fListaCamposNome.Strings[i]) is TComboBox) then
      TComboBox(FindComponent('edtLanc_' + fListaCamposNome.Strings[i])).ItemIndex := 0;
  end;
end;
