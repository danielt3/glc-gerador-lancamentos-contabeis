...
type
  ...
  private
    ...
    procedure CarregarListaDadosCampoLancamento(var pColumn: TColumn; pNomeCampo: String);
    procedure CarregarListaVinculadoresCampoLancamento(var pColumn: TColumn);
...
begin
  ...
  if (DataModule1.CampoLancamentoDados) then
    CarregarListaDadosCampoLancamento(lColumn, DataModule1.CampoLancamentoDescricao)
  else if (DataModule1.CampoLancamentoNome = 'vinculador') then
    CarregarListaVinculadoresCampoLancamento(lColumn);
...
procedure TfrmPrincipal.CarregarListaDadosCampoLancamento(var pColumn: TColumn; pNomeCampo: String);
const
  lTabela = 'DadosCampos';
var
  lComandoSQL: String;
  teste: Integer;
begin
  pColumn.ButtonStyle := cbsPickList;
  dbgDadosCampos.DataSource := DataModule1.getDataSource(lTabela);
  ...
  while not DataModule1.getQuery(lTabela).EOF do
  begin
    pColumn.PickList.Add(DataModule1.getQuery(lTabela).FieldByName('dado').AsString);

    DataModule1.getQuery(lTabela).Next;
  end;
end;

procedure TfrmPrincipal.CarregarListaVinculadoresCampoLancamento(var pColumn: TColumn);
const
  lTabela = 'DadosCampos';
var
  lComandoSQL: String;
...
begin
  ...
  while not DataModule1.getQuery(lTabela).EOF do
  begin
    pColumn.PickList.Add(DataModule1.getQuery(lTabela).FieldByName('vinculador').AsString);
...
begin
  if (dbgLancamento.SelectedColumn.ButtonStyle = cbsPickList) and
     (dbgLancamento.SelectedColumn.PickList.IndexOf(dbgLancamento.SelectedField.AsString) < 0) then
    dbgLancamento.SelectedField.AsString := '';
end;
